...
class VoiceAnonymizer:
    def __init__(self, extra_params=None):        
        ...
        self.model = SpeechT5ForSpeechToSpeech.from_pretrained(self.model_name)
    ...
    def anonymize(self, source_files, target_files, output_files):
        ...
        for line in tqdm(zip(source_files, target_files, output_files)):
            ...
            void, target_speaker_embeddings = self.speakerEmbeddingExtractor.contextual_encoding(target_waveform)

            # Generate anonymized voice using the SpeechT5 model and HiFi-GAN vocoder
            output_audio_descriptor = self.model.generate_speech(source_audio_descriptor["input_values"].squeeze(1),
                                                            target_speaker_embeddings.squeeze(1), vocoder=self.vocoder)
